...
export default function ModelDropdown() {
  const { models, selectedModel, selectedEndpointId, setSelectedModel, loading, error, refresh } = useModel()
  ...
  const currentModel = models.find(m => m.name === selectedModel && m.endpointId === selectedEndpointId)
  const label = currentModel ? `${current} (${currentModel.endpointLabel})` : current
  ...
  const modelsByEndpoint = models.reduce((acc, model) => {
    ...
    return acc
  }, {} as Record<string, { label: string; models: typeof models }>)
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <span className="text-slate-600 font-medium truncate max-w-32">{label}</span>
      ...
      {isOpen && (
        <div className={`absolute ${shouldOpenUpward ? 'bottom-full mb-2' : 'top-full mt-2'} right-0 w-56 bg-white/95 backdrop-blur-md border border-white/40 rounded-lg shadow-elegant z-50 overflow-hidden ${shouldOpenUpward ? 'animate-fade-in-down' : 'animate-fade-in-up'}`}>
          <ul className="py-1 max-h-64 overflow-auto" aria-label="Model options">
            ...
            {!loading && Object.entries(modelsByEndpoint).map(([endpointId, endpoint]) => (
              <li key={endpointId}>
                {/* Endpoint header */}
                ...
                {endpoint.models.map(m => {
                  const isSelected = m.name === selectedModel && m.endpointId === selectedEndpointId
                  return (
                    <button
                      key={`${endpointId}-${m.name}`}
                      type="button"
                      aria-current={isSelected ? 'true' : undefined}
                      onClick={() => { 
                        setSelectedModel(m.name, m.endpointId); 
                        setIsOpen(false) 
                      }}
                      className={`w-full flex items-center justify-between px-4 py-2.5 text-sm text-left transition-colors ${isSelected ? 'bg-slate-100 text-slate-900 font-semibold' : 'text-slate-700 hover:bg-slate-50'}`}
                    >
